...
function SlotRow({slot, client, onCheck, onPartial}) {
  ...
  return <div style={{marginBottom:4}}>
    <div style={{display:"flex",alignItems:"center",gap:10,padding:"9px 12px",background:open?"rgba(99,102,241,0.08)":slot.overtime?"rgba(239,68,68,0.06)":"rgba(255,255,255,0.03)",borderRadius:open?"8px 8px 0 0":8,border:"1px solid "+(open?"rgba(99,102,241,0.2)":T.border),cursor:"pointer",transition:"all 0.15s"}}
      onClick={function(){setOpen(function(o){return !o;});}}>
      <div style={{width:22,height:22,border:"1.5px solid "+(open?"#6366f1":T.faint),borderRadius:6,flexShrink:0,background:open?"rgba(99,102,241,0.2)":"",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,color:"#6366f1",fontWeight:700,transition:"all 0.15s"}}>
        {open?"×":"✓"}
      </div>
      <div style={{minWidth:80,textAlign:"right"}}>
        <div style={{fontFamily:"'Inter',monospace",fontSize:11,fontWeight:600,color:T.text,background:"rgba(255,255,255,0.06)",borderRadius:4,padding:"2px 7px",display:"inline-block"}}>{slot.startClock}</div>
...
// Modal d'édition d'un slot (horaire + caissons + client info)
  ...
  return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={onClose}>
    <div style={{background:"#1a1a24",border:"1px solid rgba(255,255,255,0.12)",borderRadius:16,padding:24,width:"100%",maxWidth:480,boxShadow:"0 24px 64px rgba(0,0,0,0.6)",animation:"fadeIn 0.2s ease"}} onClick={function(e){e.stopPropagation();}}>
      ...
      <div style={{display:"grid",gap:14}}>
        <div>
          <div style={{fontSize:11,color:T.muted,fontWeight:500,textTransform:"uppercase",letterSpacing:"0.05em",marginBottom:6}}>Client</div>
          <input value={name} onChange={function(e){setName(e.target.value);}} placeholder="Nom du client" style={iStyle}/>
        </div>
        <div>
          <div style={{fontSize:11,color:T.muted,fontWeight:500,textTransform:"uppercase",letterSpacing:"0.05em",marginBottom:6}}>Adresse</div>
          <input value={address} onChange={function(e){setAddress(e.target.value);}} placeholder="Adresse" style={iStyle}/>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          <div>
            <div style={{fontSize:11,color:T.muted,fontWeight:500,textTransform:"uppercase",letterSpacing:"0.05em",marginBottom:6}}>Ville</div>
            <input value={ville} onChange={function(e){setVille(e.target.value);}} placeholder="Ville" style={iStyle}/>
          </div>
          <div>
            <div style={{fontSize:11,color:T.muted,fontWeight:500,textTransform:"uppercase",letterSpacing:"0.05em",marginBottom:6}}>Bon / Réf</div>
            <input value={bon} onChange={function(e){setBon(e.target.value);}} placeholder="Réf…" style={iStyle}/>
          </div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          <div>
            <div style={{fontSize:11,color:T.muted,fontWeight:500,textTransform:"uppercase",letterSpacing:"0.05em",marginBottom:6}}>Heure de début</div>
            <div style={{display:"flex",gap:6,alignItems:"center"}}>
              <input type="number" min="7" max="18" value={startH} onChange={function(e){setStartH(e.target.value.padStart(2,"0"));}} style={Object.assign({},iStyle,{width:60,textAlign:"center"})}/>
          ...
          <div>
            <div style={{fontSize:11,color:T.muted,fontWeight:500,textTransform:"uppercase",letterSpacing:"0.05em",marginBottom:6}}>Nb caissons</div>
      ...
      <div style={{display:"flex",gap:8,marginTop:20,justifyContent:"space-between"}}>
        <Btn onClick={onRemove} variant="danger" size="sm">🗑 Retirer du planning</Btn>
        <div style={{display:"flex",gap:8}}>
...
// Modal pour ajouter un chantier existant à une journée
  ...
  return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={onClose}>
    <div style={{background:"#1a1a24",border:"1px solid rgba(255,255,255,0.12)",borderRadius:16,padding:24,width:"100%",maxWidth:480,boxShadow:"0 24px 64px rgba(0,0,0,0.6)",animation:"fadeIn 0.2s ease"}} onClick={function(e){e.stopPropagation();}}>
      ...
      <div style={{display:"flex",gap:8,marginTop:16,justifyContent:"flex-end"}}>
        <Btn onClick={onClose} variant="ghost" size="sm">Annuler</Btn>
        <Btn onClick={function(){if(selected)onAdd(selected);}} variant="primary" size="sm" disabled={!selected}>Ajouter à la journée</Btn>
      </div>
    </div>
  </div>;
}
